...
const generateAnswer = (question) => {
  if (question.jenis === "MULTIPLE_CHOICE" || question.jenis === "TRUE_FALSE") {
    // Pilih jawaban benar dengan probabilitas 75%, salah 25%
...
const calculateScore = (answers, questions) => {
  ...
  answers.forEach((answer, index) => {
    ...
    if (answer.adalahBenar === true) {
      ...
      // Essay, berikan nilai random 60-95% dari poin
...
async function main() {
  console.log("🌱 Starting seed for Penjas & TIK by Erna with Student Submissions (Semester Genap 2025-2026)...\n");
  ...
  for (const cst of classSubjectTutors) {
    ...
    console.log(`\n📝 Processing ${subjectName} @ ${className} (${students.length} students)...\n`);

    // ========== 2.1 UTS (Ujian Tengah Semester) ==========
    console.log("  Creating/Finding UTS (Midterm Exam)...");
    let uts = await prisma.assignment.findFirst({
      where: {
    ...
    if (!uts) {
      uts = await prisma.assignment.create({
        data: {
      ...
      console.log(`  ⏭️  UTS already exists: ${uts.judul}`);
    }

    // Generate submissions untuk UTS
    console.log(`  📤 Generating UTS submissions for ${students.length} students...`);
    for (const student of students) {
      ...
      if (!existingSubmission) {
        const answers = uts.questions.map((q) => generateAnswer(q));
        const nilai = calculateScore(answers, uts.questions);

        const waktuMulai = new Date("2026-03-15T08:00:00");
        const waktuKumpul = new Date(waktuMulai.getTime() + Math.random() * 90 * 60000);
        ...
        await prisma.submission.create({
          data: {
            ...
            answers: {
              create: uts.questions.map((question, idx) => ({
    ...
    console.log(`  ✅ UTS submissions completed\n`);

    // ========== 2.2 UAS (Ujian Akhir Semester) ==========
    console.log("  Creating/Finding UAS (Final Exam)...");
    let uas = await prisma.assignment.findFirst({
      where: {
        classSubjectTutorId: cst.id,
        jenis: "FINAL_EXAM",
      },
    ...
    if (!uas) {
      uas = await prisma.assignment.create({
        data: {
      ...
      console.log(`  ⏭️  UAS already exists: ${uas.judul}`);
    }

    // Generate submissions untuk UAS
    console.log(`  📤 Generating UAS submissions for ${students.length} students...`);
    for (const student of students) {
      const existingSubmission = await prisma.submission.findFirst({
        where: {
          studentId: student.id,
      ...
      if (!existingSubmission) {
        const answers = uas.questions.map((q) => generateAnswer(q));
        const nilai = calculateScore(answers, uas.questions);

        const waktuMulai = new Date("2026-06-15T08:00:00");
        const waktuKumpul = new Date(waktuMulai.getTime() + Math.random() * 120 * 60000);
        ...
        await prisma.submission.create({
          data: {
            ...
            waktuDinilai: waktuKumpul,
            answers: {
              create: uas.questions.map((question, idx) => ({
                questionId: question.id,
                jawaban: answers[idx].jawaban,
                adalahBenar: answers[idx].adalahBenar,
                nilai:
    ...
    console.log(`  ✅ UAS submissions completed\n`);

    // ========== 2.3 TUGAS (Exercise) ==========
    console.log("  Creating/Finding Tugas (Exercise)...");
    let tugas = await prisma.assignment.findFirst({
      where: {
        classSubjectTutorId: cst.id,
        jenis: "EXERCISE",
      },
    ...
    if (!tugas) {
      tugas = await prisma.assignment.create({
        data: {
      ...
      console.log(`  ⏭️  Tugas already exists: ${tugas.judul}`);
    }

    // Generate submissions untuk Tugas
    console.log(`  📤 Generating Tugas submissions for ${students.length} students...`);
    for (const student of students) {
      const existingSubmission = await prisma.submission.findFirst({
        where: {
          studentId: student.id,
      ...
      if (!existingSubmission) {
        const answers = tugas.questions.map((q) => generateAnswer(q));
        const nilai = calculateScore(answers, tugas.questions);

        const waktuMulai = new Date("2026-02-01T08:00:00");
        const waktuKumpul = new Date("2026-02-14T15:30:00");
        ...
        await prisma.submission.create({
          data: {
            ...
            waktuKumpul,
            nilai,
            waktuDinilai: new Date("2026-02-15T10:00:00"),
            answers: {
              create: tugas.questions.map((question, idx) => ({
                questionId: question.id,
                jawaban: answers[idx].jawaban,
                adalahBenar: answers[idx].adalahBenar,
                nilai:
    ...
    console.log(`  ✅ Tugas submissions completed\n`);

    // ========== 2.4 UJIAN HARIAN (Daily Test) ==========
    console.log("  Creating/Finding Ujian Harian (Daily Test)...");
    let ujianHarian = await prisma.assignment.findFirst({
      where: {
        classSubjectTutorId: cst.id,
        jenis: "DAILY_TEST",
      },
    ...
    if (!ujianHarian) {
      ujianHarian = await prisma.assignment.create({
        data: {
          judul: subjectName.includes("Jasmani")
      ...
      console.log(`  ⏭️  Ujian Harian already exists: ${ujianHarian.judul}`);
    }

    // Generate submissions untuk Ujian Harian
    console.log(`  📤 Generating Ujian Harian submissions for ${students.length} students...`);
    for (const student of students) {
      const existingSubmission = await prisma.submission.findFirst({
        where: {
          studentId: student.id,
          assignmentId: ujianHarian.id,
        },
      });

      if (!existingSubmission) {
        const answers = ujianHarian.questions.map((q) => generateAnswer(q));
        const nilai = calculateScore(answers, ujianHarian.questions);

        const waktuMulai = new Date("2026-02-20T08:00:00");
        const waktuKumpul = new Date(waktuMulai.getTime() + Math.random() * 45 * 60000);
        ...
        await prisma.submission.create({
          data: {
            ...
            waktuDinilai: waktuKumpul,
            answers: {
              create: ujianHarian.questions.map((question, idx) => ({
                questionId: question.id,
                jawaban: answers[idx].jawaban,
                adalahBenar: answers[idx].adalahBenar,
                nilai:
    ...
    console.log(`  ✅ Ujian Harian submissions completed\n`);

    // ========== 2.5 KUIS (Quiz) ==========
    console.log("  Creating/Finding Kuis (Quiz)...");
    let kuis = await prisma.quiz.findFirst({
      where: {
        classSubjectTutorId: cst.id,
      },
      include: {
    ...
    if (!kuis) {
      ...
      console.log(`  ⏭️  Kuis already exists: ${kuis.judul}`);
    }

    // Generate submissions untuk Kuis
    console.log(`  📤 Generating Kuis submissions for ${students.length} students...`);
    for (const student of students) {
      const existingSubmission = await prisma.submission.findFirst({
        where: {
          studentId: student.id,
      ...
      if (!existingSubmission) {
        const answers = kuis.questions.map((q) => generateAnswer(q));
        const nilai = calculateScore(answers, kuis.questions);

        const waktuMulai = new Date("2026-03-01T08:00:00");
        const waktuKumpul = new Date(waktuMulai.getTime() + Math.random() * 20 * 60000);
        ...
        await prisma.submission.create({
          data: {
            ...
            waktuDinilai: waktuKumpul,
            answers: {
              create: kuis.questions.map((question, idx) => ({
                questionId: question.id,
                jawaban: answers[idx].jawaban,
                adalahBenar: answers[idx].adalahBenar,
                nilai:
    ...
    console.log(`  ✅ Kuis submissions completed\n`);
  }
  ...
  console.log(`   - Total Assignments/Quizzes Created: ${totalAssignmentsCreated}`);
  console.log(`   - Total Student Submissions Created: ${totalSubmissionsCreated}`);
  console.log(`\n✅ All data successfully seeded with proper database mappings!`);
}
